...
return function()
  local statusline = require('v.utils.statusline')
  ...
  require('lualine').setup({
    ...
    sections = {
      lualine_a = {
        {
          ...
          fmt = function(str)
            ...
            local cwd = statusline.get_cwd()
            local dirs = statusline.convert_path_to_list(str)
      ...
      lualine_b = {
        {
          ...
          fmt = function(str)
            ...
            local cwd = statusline.get_cwd()
            local nodes = statusline.convert_path_to_list(str)
      ...
      lualine_z = {
        ...
        {
          function()
            local curr, _ = statusline.get_line_extremes()
            return curr
          end,
          color = { gui = 'bold' },
          padding = 0,
        ...
        {
          function()
            local _, last = statusline.get_line_extremes()
            return '/' .. last
          end,
          padding = { left = 0, right = 1 },
          color = { fg = colors.grey_light, gui = 'italic' },
      ...
      lualine_a = {
        {
          'filename',
          fmt = function(str)
            return str:gsub('(.+%/)[^/]+(%/[^/]+)$', '%1' .. statusline.get_current_files_dir() .. '%2')
          end,
          path = 1,
        },
      },
